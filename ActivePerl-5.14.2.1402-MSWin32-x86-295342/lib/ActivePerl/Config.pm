...
sub override {
	...
	if ($Config::Config{ccname} eq "gcc") {
	    ...
	    unshift(@make, "nmake") if _orig_conf("cc") eq "cl" && ActiveState::Path::find_prog("cl");
	}
	for (@make) {
	    ...
	    _orig_conf("cc") eq "cl" && (my $cl = ActiveState::Path::find_prog("cl")))
	{
	    ...
	    my($gccversion) = qx(gcc --version 2>/dev/null);
	    if ($gccversion) {
		...
		$gccversion = _orig_conf("gccversion");
	    }
	    ...
	    if ($sdk && !-d $sdk) {
		chomp(my $arch = qx(/usr/bin/arch));
		# our binaries contain 'ppc' and 'i386' support.  If `arch` returns
		# something else lets at least hope the machine continues to be able to
		# handle "i386" binaries
		$arch = "i386" unless $arch eq "i386" || $arch eq "ppc";
		warn "Set up build environment without $sdkv SDK (will build $arch binaries)\n"
		    unless $ENV{ACTIVEPERL_CONFIG_SILENT};
		my $sdk_re = qr/$sdk|-nostdinc|-no-cpp-precomp|-mmacosx-version-min/;

		foreach my $flag (keys %flags) {
		    $flags{$flag} =~ s/(-arch\s+(\w+)\s*)/$2 eq $arch ? $1 : ""/ge;
