...
sub ldopts {
	...
	$libperl = (grep(/^-l\w*perl\w*$/, @link_args))[0]
		...
		? "-l$1" : '')
	    || "-lperl";
	if ($libperl eq "-lperl" && $Config{libperl} eq "libperl.a" &&
	    -f File::Spec->catfile($lpath, "libperl.so"))
	{
	    $libperl = File::Spec->catfile($lpath, $Config{libperl});
	    $lpath = undef;
	}
    }

    my($extralibs, $bsloadlibs, $ldloadlibs, $ld_run_path) =
	MM->ext(join ' ', ($lpath ? "-L$lpath" : ()), $libperl, @potential_libs);
