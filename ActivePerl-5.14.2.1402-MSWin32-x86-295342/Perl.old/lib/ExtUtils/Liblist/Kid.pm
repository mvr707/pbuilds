...
sub _win32_ext {
	...
	# do still another pass in case we are compiling with $GC but
	# are looking for a lib compiled with $VC
	if (!$found_lib and $GC and !$thirdpass++) {
	    s/^lib//i;
	    goto LOOKAGAIN if s/\Q$libext\E$/.lib/;
	}
