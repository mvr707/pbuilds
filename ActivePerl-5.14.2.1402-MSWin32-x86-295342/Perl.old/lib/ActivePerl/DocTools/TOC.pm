...
my %rename = (
    ...
    perl5db   => ($^O eq "darwin" || $^O eq "MSWin32" && Win32::BuildNumber() >= 821 ? "pods::" : "Pod::") . "perl5db",
    perllocal => ($^O eq "darwin" || $^O eq "MSWin32" && Win32::BuildNumber() >= 821 ? "pods::" : "Pod::") . "perllocal",
    # because podlators from CPAN will install into Pod::perlpodstyle
    "Pod::perlpodstyle" => ($^O eq "darwin" || $^O eq "MSWin32" && Win32::BuildNumber() >= 821 ? "pods::" : "Pod::") . "perlpodstyle",
